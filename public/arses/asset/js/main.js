$(document).ready(function () {
    ...
    $(window).on("resize", function () {
        $(".discountBx").slideUp();
        $(".discountCod p").fadeIn();
        $(".discountCod").removeClass("clicked");
    });

    if ($(".discountCod").length) {
        $(".discountCod").each(function () {
            var thisBtn = $(this);
            var thisBtnP = $(this).find("p");
            thisBtnP.click(function (ev) {
                ev.preventDefault();
                $(".discountBx").slideUp();
                thisBtnP.next(".discountBx").fadeIn(300);
